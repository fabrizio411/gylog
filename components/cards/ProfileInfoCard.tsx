...
const ProfileInfoCard: React.FC<ProfileInfoCardProps> = ({
  ...
  return (
    <article className='relative flex flex-col items-center mt-3 sm:mt-5 w-11/12 sm:w-full'>
      <Image className='rounded-full' src={image || '/images/placeholder.jpg'} alt='profile photo' height={90} width={90} />
      <h3 className='mt-3 text-light-1 font-semibold text-3xl'>{username}</h3>
      <p className='text-xs text-light-3'>Joined: {formatedDate}</p>
